...
get.stats.fn = function(dir.hr, dir.step, smth.k.par = 10, rest.min.par = 10, base.num.par=28, resol.tm.par=60, test.r.fixed.par=FALSE, test.r.par=NA, res.quan.par=0.9, pval.thres.par=0.01) {
...
stats.fn = function (hr.mx, dat.step.1, X.conv.f, base.num=28, resol.tm.1=60, test.r.fixed.1=FALSE, test.r.1=NA, res.quan.1=0.9, pval.thres=0.01) {
    ...
    test.r.1 = base.info$test.r
	base.res = base.info$z.v		
    test.r.seq = rep(test.r.1, length(base.res))
	...
	for (k in 1:length(day.test)) {
        ...
        if (sum(!is.na(x.res)) >= 2 ) { 
            res.t = c(res.t, x.res)
	        x.test.r = rep(test.r.1, length(x.res))
	        names(x.test.r) = names(x.res)
            test.r.seq = c(test.r.seq, x.test.r)
            
			# to obtain cusum stats
 			x.test = udpate.cusum.fn(x.res, test.t, test.r=test.r.1)			
	...
	return(list(res.t = res.t, test.t=test.t, test.t.ind=test.t.ind, risk.score=risk.score, test.r.seq= test.r.seq))
}
...
#########################################
cusum.detection.fn = function (test.t, test.t.ind, risk.score, pval.thres=0.01, start.hour=6, dur.hour=12) {
   test.t = cusum.t
   test.t.ind = cusum.t.ind
   risk.score = cusum.risk.score
    ...
    if (!is.null(test.alarming.tm)) {
	    ...
	    alarm.tab = alarm.tab[as.numeric(alarm.tab[,6]) > dur.hour & as.numeric(alarm.tab[,4]) > start.hour, ]
	    ...
	    alarm.sum = cbind(alarm.tab[, "alarming.time"], test.sum[alarm.tab[, "alarming.time"], 2],          test.sum[alarm.tab[, "anormaly_max_time"], 2], alarm.tab[,"total_hours"],  round(as.numeric(test.sum[alarm.tab[, "alarming.time"], 1]), 2), round(1 - as.numeric(test.sum[alarm.tab[, "alarming.time"], 3]), 2) )
	    colnames(alarm.sum) = c("alarming time", "alarming hour", "duration hours to max", "total hours", "CuSum statistics at alarm", "p-value")
...
#### rank scan detection
rhr.diff.detection.fn = function (res.t, alpha=0.05, B=1000) {
	timerange1 = "20160106 0000/20160106 2359"
	min.nm = format(timeBasedSeq(timerange1), "%H:%M:%S")
	...
	detect = cbind(names(z.v)[result[,1]], names(z.v)[result[,2]], result[,3])
	if (ncol(detect) == 1) {
		detect = NULL
	} else {
		detect = detect[!duplicated(detect[,3]), ]
		detect = matrix(detect, ncol=3)	
		...
		detect = matrix(detect, ncol=3)
		detect.pos = matrix(detect[detect[,3] > 0, -3],ncol=2)
		colnames(detect.pos) = c("staring time of detected interval", "ending time of detected interval")
	}
	return(detect.pos)
}
...
result.plot.fn = function (res.t, cusum.t, cusum.test.r,  offline.result, cusum.alarm.result) {

    pdf(file="detectoin_plot.pdf", width=17, height=8)
    par(mfrow=c(2,1))
    ...
    names(res.t.1) = nm.dum
    res.t.1[names(res.t)] = res.t
	plot(res.t.1, type="l", xaxt="n", xlab="", ylab="residuals", cex.lab=2, cex.main=2, main="offline detection") 
	...
	test.t = cusum.t
	test.alarming.tm = cusum.alarm.result
	plot(test.t, type="l", xaxt="n", xlab="", ylab="CuSum statistics", cex.lab=2, cex.main=2, main="online detection") 
	...
	abline(v=pos[test.alarming.tm[,1]], col="blue", lwd=2, lty=2 )
    dev.off()
